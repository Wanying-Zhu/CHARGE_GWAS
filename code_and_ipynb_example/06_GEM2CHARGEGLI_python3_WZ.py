...
if not "chr22" in infile1 and not "chr22" in infile2 and not "chr22" in snpinfofile:
    ...
    zero = ''
    one = ''
    if "AF_" + Ename + "_0" in columns1 or "AF_" + Ename + "_1" in columns1 or "N_" + Ename + "_1" in columns1:
        zero = '_0'
        one = '_1'
    elif "AF_" + Ename + "_0.0" in columns1 or "AF_" + Ename + "_1.0" in columns1 or "N_" + Ename + "_1.0" in columns1:
        zero = '_0.0'
        one = '_1.0'
    ##########################

    if not "AF_" + Ename + zero in columns1 or not "AF_" + Ename + one in columns1 or not "N_" + Ename + one in columns1:
        quantE = True
    ...
    while line1 and line2:
        ...
        if quantE:
            ...
            outfile_handle.write(str.encode(fields1[columns1["SNPID"]]) + b"\t" + str.encode(fields1[columns1["CHR"]]) + b"\t" + str.encode(fields1[columns1["POS"]]) + b"\t" + str.encode(info[fields1[columns1["SNPID"]]]) + b"\t" + str.encode(imputed[fields1[columns1["SNPID"]]]) + b"\t" + str.encode(fields1[columns1["Effect_Allele"]]) + b"\t" + str.encode(fields1[columns1["Non_Effect_Allele"]]) + b"\t" + str.encode(fields1[columns1["AF"]]) + b"\t" + str.encode(fields1[columns1["AF_" + Ename + zero]]) + b"\t" + str.encode(fields1[columns1["AF_" + Ename + one]]) + b"\t" + str.encode(fields1[columns1["N_Samples"]]) + b"\t" + str.encode(fields1[columns1["N_" + Ename + one]]) + b"\t" + str.encode(fields2[columns2["Beta_Marginal"]]) + b"\t" + str.encode(fields2[columns2["SE_Beta_Marginal"]]) + b"\t" + str.encode(fields2[columns2["P_Value_Marginal"]]) + b"\t" + str.encode(fields1[columns1["Beta_G"]]) + b"\t" + str.encode(fields1[columns1["SE_Beta_G"]]) + b"\t" + str.encode(p_snp_m1_mb) + b"\t" + str.encode(fields1[columns1["robust_SE_Beta_G"]]) + b"\t" + str.encode(p_snp_m1_robust) + b"\t" + str.encode(fields1[columns1["Beta_G-" + Ename]]) + b"\t" + str.encode(fields1[columns1["SE_Beta_G-" + Ename]]) + b"\t" + str.encode(fields1[columns1["P_Value_Interaction"]]) + b"\t" + str.encode(fields1[columns1["robust_SE_Beta_G-" + Ename]]) + b"\t" + str.encode(fields1[columns1["robust_P_Value_Interaction"]]) + b"\t" + str.encode(fields1[columns1["P_Value_Joint"]]) + b"\t" + str.encode(fields1[columns1["Cov_Beta_G_G-" + Ename]]) + b"\t" + str.encode(fields1[columns1["robust_P_Value_Joint"]]) + b"\t" + str.encode(fields1[columns1["robust_Cov_Beta_G_G-" + Ename]]) + b"\n")
